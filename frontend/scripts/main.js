    ...
    let map, userMarker, parkingData = [], markers = [], infoWindow, bookings = [];
    ...
    function openBookingModal(item){
      ...
      document.getElementById('confirm-book').addEventListener('click', () => {
        ...
        bookings.push({ ...item, bookingId, startTime, endTime, numHours, totalFare });
        renderBookings(); // Update bookings tab
    ...
    function renderBookings() {
      const bookingsListEl = document.getElementById('bookings-tab');
      bookingsListEl.innerHTML = '';

      if (bookings.length === 0) {
        bookingsListEl.innerHTML = '<div style="margin-top:10px;color:var(--muted);font-size:13px">Your bookings will appear here.</div>';
      } else {
        bookings.forEach(booking => {
          const el = document.createElement('div');
          el.className = 'card';
          el.innerHTML = `
            <h4>${escapeHtml(booking.Location)}</h4>
            <div class="muted">Booking ID: ${escapeHtml(booking.bookingId)}</div>
            <div class="muted">Time: ${escapeHtml(booking.startTime)} - ${escapeHtml(booking.endTime)} (${escapeHtml(booking.numHours)} hrs)</div>
            <div class="muted">Total: ${escapeHtml(booking.totalFare)}</div>
          `;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  tabButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (targetTab === 'parking' && map) {
        // Trigger map resize and recenter after the tab content becomes visible
